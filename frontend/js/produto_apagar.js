...
btn_apagar.addEventListener('click', () => {
    ...
    if (!codigo) {
        resposta.innerHTML = '<p style="color: #ffaa00;">Informe o código do produto que deseja apagar!</p>'
        return
    }

    const confirmacao = confirm(`Tem certeza que deseja apagar o produto de código ${codigo}?`)
    if (!confirmacao) {
    ...
    .catch(err => {
        ...
        resposta.innerHTML = '<p style="color: red;">Erro ao apagar o produto no servidor local.</p>'
    })
})
