...
--
-- Current Database: `db_compras`
--

CREATE DATABASE /*!32312 IF NOT EXISTS*/ `db_compras` /*!40100 DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci */ /*!80016 DEFAULT ENCRYPTION='N' */;
...
--
-- Dumping events for database 'db_compras'
--

--
-- Dumping routines for database 'db_compras'
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`%` SQL SECURITY DEFINER */
/*!50001 VIEW `vw_volume_compras` AS select `p`.`nome` AS `nome`,sum(`c`.`quantidadeMovimentada`) AS `quantidade_total_movimentada`,sum((`c`.`quantidadeMovimentada` * `c`.`precoUnitario`)) AS `valor_financeiro_movimentado` from (`compras` `c` join `produtos` `p` on((`c`.`idProduto` = `p`.`codProduto`))) where (`c`.`tipoMovimento` = 'SAIDA') group by `p`.`codProduto`,`p`.`nome` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2026-08-10 21:52:47
